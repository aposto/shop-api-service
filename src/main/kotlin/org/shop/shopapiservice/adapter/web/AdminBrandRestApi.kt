...
class AdminBrandRestApi(
    ...
    @PostMapping
    @ResponseBody
    suspend fun createBrand(@RequestBody brand: ModifyBrandRequest, response: ServerHttpResponse): CommandBodyResult<BrandDetail> {
        return try {
    ...
    @PutMapping("/{brandId}")
    @ResponseBody
    suspend fun updateBrand(@PathVariable("brandId") brandId: Int, @RequestBody brand: ModifyBrandRequest): CommandBodyResult<Boolean> {
        return try {
